# First of all
*Most* of the implementation of the web application itself was part of the university course project deliverable.
The HTTPS/SSL and containerization of the application was done by myself in my own time.

The following is the setup guide pre-dating the containerization of the application. 

# Getting started

1. Clone this repository and cd into `Team4`
2. This was developed on linux (ubuntu 22.04), which will be the main focus.
...
# Setup
## backend
1. cd into the **backend** folder, here we are going to setup a python virtual environment, **venv** where we are going to keep the necessary dependencies. In **backend** run: `python3 -m venv [your venv name]` or `python -m venv [your venv name]`, You can place it where you'd like, but we recommend placing it in the **backend/** or **backend/ServerBackend/** folder.
2. Activate the venv using `source [your venv name]/bin/acivate` (Linux) or `source [your venv name]/Scripts/acivate` (Windows)
3. From within the venv and the **backend** folder, install the requirements using pip install -r requirements.txt. If you're using python 3.10.12 all pip installs should be successful
4. all backend dependencies should now be installed in your venv folder.
## frontend
...
3. Setup database migrations by running the command: `python manage.py makemigrations` 
4. Make the database migrations by running the command: `python manage.py migrate`
5. Populate the tasks (games challenges): `python manage.py populate_tasks`
6. Run the backend server using the command: `python manage.py runserver`
7. The server is running if it says **`Quit the server with CONTROL-C`** at the last line
8. To use the server, the terminal and proccess must remain open

### Celery and redis
1. Open up a new terminal
2. cd into `backend/ServerBackend`
...
1. Open up a new terminal
2. cd into `frontend/src`
3. run `npm start`
4. a new browser tab should open with the react frontend.
...
### How to merge main into my local branch the changes that other made in the main branch
...
3.  Pull the changes into our local main branch: `git pull origin main`
    ...
    We can stash our changes if we're in the middle of something and not ready to commit. We can always apply our stashed changes after pulling the updates from main.
    Stash example: `git add frontend/src/App.js`, `git add frontend/src/components/FrontPage.js` and `git stash push -m "Describe our changes"`
